...
1. `EntwicklungsprozessService.ProzessStartenAsync(...)` klont das Repository (`PrepareCloneDirectoryAsync`) und ruft
   direkt danach — noch vor der Branch-Erstellung — `GitOrchestrationService.ValidateWorkingDirectoryAfterCloneAsync(clonePath, repository.StartKonfiguration)`
   auf. `GitOrchestrationService` wird dazu als optionale Abhängigkeit über
   `EntwicklungsprozessServiceOptions.GitOrchestrationService` injiziert (WPF-DI: `App.xaml.cs`); ist sie nicht
   gesetzt, entfällt dieser frühe Prüfschritt ersatzlos (Rückwärtskompatibilität für Aufrufer/Tests ohne diese
   Abhängigkeit).
2. Falls `RepositoryStartKonfiguration.WorkingDirectoryRelativePath` gesetzt:
   ...
   - Falls Pfad ungültig: `InvalidOperationException` werfen, Klon als fehlgeschlagen markieren
   - Falls Verzeichnis nicht existiert: `DirectoryNotFoundException` werfen, Fehler protokollieren
3. Falls Validierung bestanden: Klon ist erfolgreich, Verzeichnis steht zur Verfügung. Diese frühe Prüfung ist eine
   Ergänzung zur ohnehin am CLI-Start durchgeführten Prüfung in `KiAusfuehrungsService` (über denselben
   `WorkingDirectoryResolver`) — sie liefert lediglich ein früheres, klareres Fehlerbild direkt nach dem Klon,
   bevor Branch/Startskript/CLI-Start überhaupt versucht werden.

Beteiligte Klassen/Komponenten: `EntwicklungsprozessService`, `GitOrchestrationService`, `KiAusfuehrungsService`, `RepositoryStartKonfiguration`
...
- **Abhängigkeit von Git-Plugin API:** Feature setzt voraus, dass `IGitPlugin` eine Methode `GetRepositoryStructureAsync()` oder ähnlich bereitstellt. Umgesetzt: Das Interface wurde um `GetRepositoryStructureAsync()` mit `NotSupportedException`-Default-Implementierung erweitert; vollständig implementiert ist sie ausschließlich für `LocalDirectoryPlugin` (siehe „Offener Punkt 1" unten). Kein offenes Risiko mehr.
...
1. Git-Plugin API wird um `GetRepositoryStructureAsync()` erweitert. Vollständig implementiert wird sie für `LocalDirectoryPlugin` (lokaler Dateisystemzugriff). Remote-Provider-Plugins (GitHub, BitBucket) behalten die `NotSupportedException`-Default-Implementierung, da `GetAvailableRepositoriesAsync` bei ihnen nur Remote-URLs ohne garantierten lokalen Klon-Pfad liefert; die Verzeichnisauswahl fällt für diese Provider automatisch auf Root (`.`) zurück.
...
- **Git-Plugin Integration (Offener Punkt 1):** Umgesetzt für `LocalDirectoryPlugin` (lokaler Dateisystemzugriff, rekursiv bis `MaxDepth`). Die Remote-Provider-Plugins `GitHubPlugin` und `BitBucketPlugin` behalten bewusst die `NotSupportedException`-Default-Implementierung aus `IGitPlugin`, da sie über `GetAvailableRepositoriesAsync` nur Remote-URLs ohne garantierten lokalen Klon liefern; die UI fällt für diese Provider automatisch auf den Root-Eintrag (`.`) zurück (kein GitLab-Plugin in diesem Repository vorhanden).
