...
- [ ] `PersistenzModus.SessionReset` / `SkillStatus.Review` — **Entscheidung (2026-08-21): Eindeutschen.** `SessionReset` → `SitzungZuruecksetzen`, `Review` → `Pruefung`. Alle Verwendungsstellen (Enum-Definition, Switch/Vergleiche, XAML-Bindings, `state.json`-Serialisierung falls als String persistiert, Tests) entsprechend anpassen.
