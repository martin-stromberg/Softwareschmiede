...
- **ComboBox für Arbeitsverzeichnis:** identisches Verhalten wie im Zuweisungsdialog — Liste beginnt mit
  `"."` (Repository-Root), gefolgt von den erfolgreich geladenen Unterverzeichnissen.
- **TextBox für manuelle Eingabe:** sichtbar, wenn die Verzeichnisstruktur nicht geladen werden kann oder
  vom Plugin nicht unterstützt wird.
...
- `DirectoryStructureBrowserService?` (optional — `null` führt zum manuellen Eingabemodus, siehe unten)
...
| `IsWorkingDirectoryManualInput` | `bool` | Zeigt an, dass statt der ComboBox die manuelle TextBox verwendet wird |
| `WorkingDirectoryInputText` | `string?` | TextBox-Wert für den manuellen relativen Pfad |
...
2. Bei erfolgreichem Laden: `AvailableWorkingDirectories` wird befüllt (mind. `"."`) und
   `IsWorkingDirectoryManualInput` bleibt `false`.
3. Ist `currentWorkingDirectory` gesetzt und nicht `"."`, aber nicht Teil der erfolgreich geladenen Liste
   (z. B. weil das Verzeichnis inzwischen umbenannt wurde), wird es trotzdem zur Liste hinzugefügt — die
   bestehende Auswahl darf beim Öffnen des Dialogs nie verloren gehen.
4. `SelectedWorkingDirectory` wird auf `currentWorkingDirectory` (falls vorhanden) oder `"."` gesetzt.
5. Ist kein Service/Plugin vorhanden oder liefert der Strukturabruf `Failed`/`NotSupported`, wird
   `IsWorkingDirectoryManualInput = true` gesetzt. `WorkingDirectoryInputText` enthält dann das aktuell
   konfigurierte Arbeitsverzeichnis oder `"."`.
...
- **`LoadWorkingDirectoriesAsync(...)`** — ruft
  `DirectoryStructureBrowserService.GetDirectoryLoadResultAsync(...)` auf und liefert ein
  `WorkingDirectoryLoadResult`. Bei erfolgreichem Abruf enthält es die Liste inklusive vorangestelltem
  `"."`-Eintrag. Bei `Failed` oder `NotSupported` enthält es `RequiresManualInput = true`; technische
  Fehler werden geloggt, aber nicht zum Dialogabbruch.
- **`LoadWithLoadingStateAsync(...)`** — kapselt den in beiden ViewModels identischen Wrapper: setzt den
  ...
  erwarteten Abbruch ab. Liefert bei Abbruch `null` zurück (statt eines Lade-Ergebnisses) — **die Anwendung
  des Ergebnisses (Auswahlmodus, manueller Modus, Vorauswahl) bleibt bewusst beim jeweiligen Aufrufer**, da
  sich dieses Verhalten zwischen den beiden ViewModels unterscheidet:
...
Bestätigt der Benutzer den Dialog (`BestaetigenCommand`), wird im manuellen Modus zuerst
`WorkingDirectoryInputText` validiert, normalisiert und nach `SelectedWorkingDirectory` synchronisiert.
Danach ruft `ProjectDetailViewModel.ArbeitsverzeichnisBearbeitenAsync`
...
Leere Eingaben werden als `"."` gespeichert. Absolute Pfade, Pfade mit `..`-Segmenten und Pfade mit
ungültigen Zeichen werden nicht akzeptiert.
...
| Fehler beim Strukturabruf | Manueller Modus startet mit `"."` | Manueller Modus startet mit dem aktuell gespeicherten Arbeitsverzeichnis |
...
- `ArbeitsverzeichnisBearbeitenViewModelTests` (`src/Softwareschmiede.Tests/App/ViewModels/`) — deckt
  Laden mit/ohne Treffer, Vorauswahl des aktuellen Verzeichnisses, manuellen Fallback, Validierung,
  Lade-Status-Toggle und das Verhalten bei Abbruch während des Ladens ab.
