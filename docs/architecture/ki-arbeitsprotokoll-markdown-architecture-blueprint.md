...
Jeder neue KI-Protokolleintrag beginnt mit einer Datumszeile im Format `# yyyy-MM-dd`, enthält eine nachvollziehbare RunId-Metadatenzeile und trennt die Antwort in eigenständige `## Schritt n`-Abschnitte.
...
| Deterministische Schritttrennung | Nicht-leere Antwortzeilen werden in `## Schritt {i+1}` überführt; bei leerer Antwort wird `## Schritt 1` mit Fallbacktext erzeugt | Stellt FR-1.2 und robuste Lesbarkeit sicher, auch bei leeren oder fehlerhaften Antworten. |
...
2. Bessere Nachvollziehbarkeit des Arbeitsverlaufs durch Schrittblöcke statt Fließtextblock.
...
|---|---|---|
| Lesbarkeit | Neue KI-Protokolle sind strukturiert, zeitlich einordenbar und schrittweise verständlich | H1-Datumszeile + standardisierte `## Schritt n`-Blöcke |
...
2. Die Schritttrennung erfolgt deterministisch über `## Schritt n`; bei leerer Antwort entsteht mindestens `## Schritt 1`.  
