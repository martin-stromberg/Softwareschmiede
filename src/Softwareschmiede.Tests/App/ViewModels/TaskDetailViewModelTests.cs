...
public sealed class TaskDetailViewModelTests : IDisposable
{
    ...
    // --- OeffneIdeCommand mit konfiguriertem Arbeitsverzeichnis ---
    ...
    public async Task OeffneIdeAsync_FindetSolutionImAufgeloestenArbeitsverzeichnis()
    {
        ...
        _db.RepositoryStartKonfigurationen.Add(new RepositoryStartKonfiguration
        {
            ...
            WorkingDirectoryRelativePath = Path.Combine("src", "solutions"),
            Aktiv = true
        });
        await _db.SaveChangesAsync();

        var aufgabe = await _aufgabeService.CreateAsync(_projektId, "Testaufgabe", "Beschreibung", repository.Id);
        await _aufgabeService.StartenAsync(aufgabe.Id, "feature/x", arbeitsverzeichnis);
