...
{
    /// <summary>IsNewer akzeptiert Versionen mit und ohne führendes v und vergleicht nach SemVer-Präzedenz.</summary>
    [Theory]
    ...
    [InlineData("1.2.3", "1.2.4-beta.1", true)]
    [InlineData("1.2.4-beta.1", "1.2.4", true)]
    [InlineData("1.2.4", "1.2.4-beta.1", false)]
    ...
    /// <summary>Vollständige SemVer-Tags inklusive Prerelease-Suffix und Metadaten werden akzeptiert; unvollständige und ungültige Tags nicht.</summary>
    [Theory]
    [InlineData("v1.2.3", true)]
    [InlineData("1.2.3+build.7", true)]
    [InlineData("v1.2.3-beta.1", true)]
    [InlineData("v1.2", false)]
    [InlineData("version-1.2.3", false)]
    public void TryParse_ShouldAcceptSemVerTags(string value, bool expected)
    {
